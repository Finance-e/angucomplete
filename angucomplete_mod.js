...
angular.module('angucomplete_mod', [] )
    .directive('angucompletemod', function ($parse, $http, $sce, $timeout) {
    return {
        restrict: 'EA',
        scope: {
            "id": "@id",
        ...
        template: '<div class="angucomplete-holder">\n\
                        <input id="{{id}}_value" ng-model="searchStr" type="text" placeholder="{{placeholder}}" class="{{inputClass}}" required/>\n\
                        <div id="{{id}}_dropdown" class="angucomplete-dropdown" ng-if="showDropdown">\n\
                            <div class="angucomplete-searching" ng-show="searching">Pesquisando...</div>\n\
                            <div class="angucomplete-searching" ng-show="!searching && (!results || results.length == 0)">Resultados não encontrados</div>\n\
